...
The conversion in still performed using ``address(x)`` and not
using ``address payable(x)``.
...
can be used to hold a sequence of bytes from one to up to 32.
...
.. note::
    The type ``byte[]`` is an array of bytes, but due to padding rules, it will waste
...
the underscores are simply ignored.
...
.. note::
    ...
    below evaluates to an integer. The fact that ``a`` is of type ``uint128`` forces the
    expression ``2.5 + a`` to have a proper type, though. Since there is no common type
    for the type of ``2.5`` and ``uint128``, the Solidity compiler will not accept
    this code.
